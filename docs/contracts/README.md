...
| `cms.contract.ts` | Editorial SEO only (`CmsSeoEntityType`, `CmsSeoByEntityResponse`, `CmsSeoPatchBody`). The drop-builder / landing-acts contracts it once defined were removed in the CMS teardown — landing pages are now code-owned (see `docs/cms-teardown-plan.md`) | `/api/cms` |
