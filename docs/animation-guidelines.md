...
## The About page — the scroll film + the Forge Altar finale

`src/features/about/` is a standalone feature (not in `landingPages/registry.ts` — About is a fixed page, never swapped like a drop). Since the 2026-08 scrollytelling redesign it **speaks the Oath's scroll-film language**: one continuous journey per breakpoint tier, behind one CMS contract.

- **The film** (`scroll/AboutScrollExperience.tsx`, ≥1280px + no reduced motion, lazy chunk): in-flow, per-section **pinned chapters** over a fixed transparent WebGL canvas — the hero cold open (SplitText forge-in, pinned descent), one full-screen chapter per CMS orb (faded full-bleed backdrop, materialize → hold → dissolve scrub, scrubbed stat count-ups, outlined ghost ordinal), the kinetic marquee ribbon, and the **Forge Altar finale**. Builders follow the Oath contract exactly: sections carry markup + `data-*` hooks only; every scrubbed move lives in `scroll/motion/build*.ts` pure builders `(host, q, motion) => disposer` under ONE `gsap.matchMedia()` (`hooks/useAboutScrollTimeline.ts`, `ABOUT_CINEMATIC_MQ` / `ABOUT_STATIC_MQ` in `aboutBreakpoints.ts`); `scroll/motion/aboutMotionState.ts` is the mutable DOM⇄WebGL bridge (scrollDepth, chapter progress, boundary bursts, altarApproach, pointer).
- **The WebGL depth layer** (`scroll/webgl/`, OathCanvasGate-pattern gate + lazy `vendor-three`): scroll IS a dolly — `AboutDepthRig` maps `scrollDepth` onto a camera z path (`aboutDepthPath.ts`) through the aurora and the scene-integrated `DustField`, arriving at the altar stage parked at the path's end. `EmberBoundaryField` is the signature transition: every chapter boundary bursts a fixed 10k pool (particle-forge standard — `aFrom→aTo` + per-seed stagger, `uMorph`/`uBurst` vocabulary) morphing between the outgoing and incoming chapters' silhouette-sampled backdrops, tinted by the incoming orb's CMS color via `resolveForgeRamp`. The DOM film stands alone without WebGL — the canvas is a layer, never a dependency.
- **The altar finale** (`altar/AltarStage.tsx`, mounted INSIDE the film's one canvas when `altarApproach` leaves zero — its `useGLTF` Suspense IS the ~2-chapters-early GLB prefetch, tracked by the `scroll/webgl/AltarLoadProgress.tsx` forging bar, which flips `data-altar-ready` for the CSS chip reveal): the grabbable anvil, hammer, per-color orb ring, and strike embers, every spatial constant stage-local so the depth rig's final knot reproduces the old standalone framing exactly. `altar/useAltarStrike.ts` runs the strike ceremony against the `ALTAR_STRIKE` / `ALTAR_FORGE` clock (`altar/altarForgeTiming.ts` — rule 5 of the particle-forge standard); at the hand-off beat the page **answers the strike with a scroll** — `onOrbStruck` → the Lenis-eased scroll back up to that orb's chapter (`scroll/hooks/useAboutOrbScrollTo.ts`, always `ScrollTrigger.getById('about-orb-pin-N').start`, never element rects, through the shared `lenisRegistry` singleton) — while the in-canvas shroud (`altar/AltarStrikeEmbers.tsx`) dissolves under the move. The canvas turns pointer-live only while the finale is pinned (`data-altar-live` CSS flip). The old strike **modal**, the DOM ember hand-off, and the altar/normal view toggle were all retired with the redesign — do not reintroduce them.
- **The static page** (`static/AboutStaticPage.tsx`): mobile/tablet, reduced-motion, and SSR/first-paint all render the same art direction without the film — full-screen faded-backdrop chapters, `RevealOnScroll` (IntersectionObserver + CSS) only — no GSAP, no pins, no WebGL.
- `content/aboutContent.schema.ts` + `resolveAboutContent.ts` — the CMS-override-with-code-defaults contract, unchanged by the redesign. The **orbs array is the content model** (add/edit/remove in `/admin/about`, tenets-style list ownership); per-orb images resolve from `mediaId` via the media index, falling back to the orb's page asset slot (`orbImage()`); the anvil/hammer GLBs stay asset-slot based via `resolveStorefrontPageAssets` (`asset_config.pages.about`). Chapters carry `id="about-orb-<id>"` (search deep links land INSIDE the film at the chapter's materialized beat) and `about:orb-N` admin preview targets on desktop and mobile alike.
