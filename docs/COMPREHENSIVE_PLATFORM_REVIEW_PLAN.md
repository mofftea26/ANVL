...
| **1.2** Admin edge off the storefront root (`F-06`) | 🟡 **Half done.** `__root.tsx` no longer statically imports `AdminAuthProvider`/`AdminThemeProvider` — the whole `/admin` branch moved behind a lazy `AdminRootShell`, and the build confirms `AdminAuthProvider` is now its own 2.13 kB chunk instead of an entry dependency. **But the entry still statically imports `admin-cms-remote`**, because `@supabase/supabase-js` lives inside that chunk and the storefront genuinely needs it. See `F-06(b)`. |
...
- **Admin shell** — the persistent sidebar/topbar **are** correctly hoisted above `<Outlet/>` (`src/routes/admin/route.tsx`). The common worry about full-shell remounts is **largely unfounded**; only one narrow residual applies (`F-14`).
...
| ID | Area | Type | Finding | Files | Conf. |
|---|---|---|---|---|---|
| **F-06** | Architecture | Performance | **(a) FIXED 2026-08-04.** The storefront root statically imported the admin auth/theme providers, putting the admin stack in every route's graph. The whole `/admin` branch now sits behind a lazy `AdminRootShell`; the build confirms `AdminAuthProvider` became its own 2.13 kB chunk. **(b) STILL OPEN — and it, not the admin import, is where the weight actually comes from:** `@supabase/supabase-js` is bundled *inside* `admin-cms-remote` (371 KB raw / 98.6 KB gz), and the storefront legitimately uses supabase-js, so the entry statically imports that chunk regardless. Root cause established by instrumenting `manualChunks` during a real build: **it is never invoked with a supabase module id at all** — Rolldown merges the package before chunking, so the `vendor-supabase` rule is unreachable code and no matcher can revive it (an attempted fix was reverted rather than shipped as an inert change). Splitting it needs a different mechanism — Rolldown `advancedChunks`, or forcing the ESM entry, since supabase-js resolves to `dist/index.cjs`. | `routes/__root.tsx`, `vite.config.ts:118` | confirmed |
...
| **F-14** | CMS | Performance | **The persistent-shell question, precisely scoped.** The shell **is** correctly hoisted; child routes use content-scoped `AdminEditorLoading`. **But** the `/admin` layout `beforeLoad` re-runs every navigation, and `defaultPendingMs: 120` (vs TanStack's 1000 default) means any check slower than 120 ms renders the layout's **full-screen `min-h-screen` `AdminRoutePending`** over the whole shell for ≥200 ms (`defaultPendingMinMs`). Fires once per 45 s cache window. | `routes/admin/route.tsx:44,32`, `router.tsx:15,16`, `admin/auth/adminAuthCache.ts:13` | confirmed (flash timing: probable) |
...
| Admin shell remounts on every editor navigation | **Largely unfounded.** Shell is hoisted above `<Outlet/>`. Only `F-14` applies. |
...
| 1.2 | ~~Remove the admin edge from the storefront root~~ → **split supabase-js out of `admin-cms-remote`** | F-06(b) | The admin edge is gone (lazy `AdminRootShell`). What remains is the chunking bug: `manualChunks` never sees supabase-js, so `vendor-supabase` cannot fire. Needs Rolldown `advancedChunks` or forcing the package's ESM entry — **not** another matcher tweak, which was tried and reverted. | M | med | ✅ |
| 1.3 | **Serve CMS images properly** | F-24 | Switch `publicCmsMediaUrl()` to Supabase's `/render/image/public/…?width=&quality=&format=` transform; add `srcSet` to the main CMS image components. | M | low | ✅ |
