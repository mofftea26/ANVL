...
> This document is a **review and a plan**, not a changelog. Nothing in it has been implemented.
> Related tracking: `docs/next-steps.md` (launch blockers), `docs/technical-debt.md` (finding IDs), `docs/audit-2026-05-17.md`.
...
| **F-04** | Supabase | Security | **Migrations no longer reproduce production.** 65 applied vs 63 files. 8 applied have no file, including **`tighten_cms_settings_rls_and_revoke_rls_auto_enable_grant`** and **`sec25_remove_public_storage_listing_policies`** — the two that closed anon read of CMS drafts and public listing of media buckets. `fix_publish_drop_body_column` was applied **twice**. A fresh `db push` rebuilds a less secure database. This is not maintenance drift; it is a regression armed to fire on the next environment rebuild. | `supabase/migrations/` (63 files) vs `list_migrations()` (65) | confirmed |
...
| ID | Area | Type | Finding | Files | Conf. |
|---|---|---|---|---|---|
| **F-06** | Architecture | Performance | Storefront root statically imports admin code, pulling `@supabase/supabase-js` (98.3 KB gz) into the always-loaded graph. Source-level violation **confirmed**; whether the browser eagerly *fetches* it needs one Network-tab check → `unverified` on that sub-claim. | `routes/__root.tsx:19,20,317`, `admin/auth/AdminAuthProvider.tsx:25-27` | confirmed / partly unverified |
...
| 1.2 | **Remove the admin edge from the storefront root** | F-06 | Wrap the admin branch of `RootLayoutBody` in a lazy `AdminRootShell` so the static import edge disappears; re-check whether `vendor-supabase` then materialises. | S | med | ✅ |
