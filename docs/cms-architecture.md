...
| About Page | `/admin/about` | `cms_settings.landing_content.about` — hero + marquee copy and the **orbs array** (free-form chapters with label/color/copy/lines/points/stats/CTAs/`mediaId`; add/edit/remove, The Oath tenets ownership contract). Each orb is a full-screen chapter of the desktop scroll film, a section on the static page, and a stone on the Forge Altar finale (strikes scroll back to the chapter). Anvil/hammer GLBs + page imagery assign on `/admin/assets` (`asset_config.pages.about`) |
