...
- **Storefront resolution:** `getResolvedStorefrontLandingCmsSync` remains the synchronous helper for loaders and inline reads. **SSR** (no browser store) still snapshots `SEED_DROP` + `SEED_WEBSITE_LAYOUT` when Supabase env is absent. **`useLandingCms` + `useActiveDrop`** on the public shell now re-read admin persistence immediately after hydrate (`useLayoutEffect`) so palette variables, emblem, nav labels, and `landingActs` match the persisted **active drop** plus website layout—the previous `initial ?? offline` ordering pinned the SSR seed Drop object and masked local updates. With **Supabase**, anonymous clients read **`storefront_publication`** via React Query; until the query resolves, loaders pass through the published SSR projection only (not admin drafts).
