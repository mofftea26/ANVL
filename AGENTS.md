...
- Never bundle **`SUPABASE_SERVICE_ROLE_KEY`**, **`ANVL_ADMIN_SESSION_SECRET`**, or other server-only secrets into client code. See **`docs/technical-debt.md`** and `CLAUDE.md` SEC-01/02/03/SEC-11 (both resolved) for the full design writeup; CSRF landed 2026-07-06 (double-submit cookie in `src/start.ts`); still open under Phase J are flipping CSP from report-only to enforcing, rate limiting, and upload validation.
