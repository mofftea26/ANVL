...
| 3D / WebGL | three.js + `@react-three/fiber` v9 + `@react-three/drei` v10 — The Oath landing emblem, Story chapter book, the About film's depth canvas (scroll-dollied camera, chapter-boundary ember bursts, the Forge Altar finale with anvil/hammer GLBs + aurora + a drei `useProgress` forging bar), and the site-wide cursor dust (`src/shared/webgl/DustField`, one shared field mounted globally via `SiteDustGate` and inside scene canvases) — all lazy `vendor-three`, gated to capable devices |
| Smooth scroll | Lenis |
...
src/features/about/**          → About page: desktop scrollytelling film (pinned full-screen orb chapters over a fixed WebGL depth canvas, ending at the interactive Forge Altar finale — hammer strikes scroll back to a chapter) + static full-screen-section page below the gate — not registered in landingPages/registry.ts (About is a fixed page, not a swappable drop)
...
src/
  ...
  features/
    ...
    about/           About page: content schema/defaults/resolver (CMS-driven orbs = chapters) + scroll/ (the desktop film — pinned chapter sections, motion builders + aboutScrollTiming clock, webgl/ depth canvas with the ember boundary field and the altar forging bar) + altar/ (the Forge Altar finale as an in-canvas stage — grabbable anvil, per-color orb ring, useAltarStrike; strikes scroll to chapters) + static/ (full-screen-section page for mobile/tablet/reduced motion)
  ...
  routes/
    ...
    about.tsx        About page — renders <AboutExperience> (desktop scrollytelling film with the Forge Altar finale / static page below the gate; CMS-editable copy + assets)
...
scripts/
  ...
  backfill-cms-image-sizes.mjs  One-time cleanup of oversized images already in `cms-media` (assets
                                predating the upload-time encode). Dry-run by default; `--apply` needs
                                SUPABASE_SERVICE_ROLE_KEY. Never deletes originals, writes a rollback manifest
...
node scripts/backfill-cms-image-sizes.mjs --filter about    # Dry-run: re-encode oversized cms-media images (--apply to write)
...
| `public.armory_events` | Append-only log of every countable action — one row per event, carrying the owner (`user_id`) AND the item (`passport_id` + denormalised `product_slug`). 12 `event_type` values feed every challenge counter. **No INSERT policy at all**: `armory_viewed` writes against the armory OWNER while the caller is someone else (often anon), which no row-level policy expresses safely, so every write goes through `record_armory_event` / `record_armory_view` (SECURITY DEFINER) and emits live inside the RPCs that already do the work. Read back via `get_armory_counters()` — 15 counters in one call, streaks as gaps-and-islands in **Asia/Beirut** (a UTC day boundary would split a Lebanese evening run in two). Replaced `passport_wear_log`, which was folded in and dropped. See `docs/backend-guidelines.md` | Owner reads own; CMS reads all; **writes RPC-only** |
| `public.gamification_settings` / `gamification_ranks` / `gamification_rank_levels` / `gamification_challenges` / `gamification_badges` | The Armory's editable gamification rules. **Rank gates on `min_xp`** since gamification v2 — a registration-gated ladder is unclimbable past the size of the catalogue, so with three products it stalled a week after launch. `deriveArmoryRank` takes `earnedXp` as a **required** parameter: an XP-only ladder has almost no count thresholds left, so skipping the gate makes every level match and returns the TOP rank to a zero-XP account (pinned by a regression test). Challenges carry `difficulty` / `xp_reward` / `tier_group` / `tier`, and `evaluateChallenges` collapses a tier family to the one tier being chased. Metric vocabulary is 20 values, not 6. (Forge XP constants + level curve; **8 seeded** rank keys with per-level AND-combined thresholds + optional `emblem_url` override — the `CHECK` constraint pinning the key set was dropped in `20260720120000_gamification_rank_keys.sql`, so admins can create and delete ranks and `ArmoryRankKey` is a free string with code-owned fallback artwork; challenges/badges as declarative `metric` + target, over the **20-value** v2 vocabulary — the original six passport-derived metrics plus wear-log-derived ones (`streak_days`, `weekly_streak`, `distinct_months`, `early_wears`) and cross-table ones (`reviews`, `public_feats`, `transfers_out`/`_in`, `tenure_days`, `armory_public`, `distinct_colorways`, `divisions_owned`, `shares`, `chapters_read`). Keep the list in sync with the CHECK constraint on `gamification_challenges.metric`; a value the DB rejects makes the admin editor able to author a challenge it cannot save). Seeded == code defaults (`DEFAULT_GAMIFICATION_RULES`), so pre-migration behavior is identical. Storefront reads via anon fetch (`useGamificationRules`, React Query, defaults as placeholder); rules resolvers in `passport/lib/{ranks,challenges,forgeXp}.ts` take rules with default fallback | Public SELECT; editor/admin write |
...
- Heavy vendors are code-split via `vite.config.ts` `manualChunks`. Chunks actually emitted by the client build (verified 2026-08-05): `admin-cms-remote`, `cms-core`, `app-runtime`, `admin-auth`, `storefront-account-client`, `vendor-gsap`, `vendor-lenis`, `vendor-three`, `vendor-pdfjs` (admin techpack parser only), `vendor-zod`, `vendor-fuse`, `vendor-react`, `vendor-sonner`, `supabase-clients`. **`vendor-supabase` and `vendor-tanstack` are requested but never emitted** — Rolldown merges any chunk that is always loaded alongside its importer, so those rules are aspirational, not effective. Do not assume a `manualChunks` name exists just because the rule is there; check `dist/client/assets/`.
- **Keeping `@supabase/supabase-js` off the eager graph is a maintained invariant.** Every path to the SDK is lazy: the `./auth` barrel must not re-export `getStorefrontSupabaseClient`/`supabaseAccountClient` (a re-export is a static edge even if nothing calls it), `storefrontAuth`/`adminCmsRemoteSync`/`mediaAssets.service` load their client via `await import(...)`, and `autoImportRun.ts` must import `cmsWriteThrough` dynamically — one static importer anywhere defeats every storefront `*.settings.ts` lazy import at once. Diagnose with `ANVL_IMPORTERS=1 pnpm build` (the module graph), never a grep: greps miss multi-line imports and `export … from` re-exports. One bundler-level edge still remains, so the SDK is *still* fetched on first paint — see `docs/deployment.md` for the async-factory change that would close it.
