...
export const badgeVariants = cva('inline-flex shrink-0 items-center rounded-full border font-semibold uppercase', {
    variants: {
        tone: {
            ...
            accent: 'border-[color-mix(in_srgb,var(--color-line)_100%,transparent)] bg-[linear-gradient(to_bottom,color-mix(in_srgb,var(--color-surface-elevated)_45%,transparent),transparent)] text-[var(--color-text-muted)]',
        },
        size: {
...
export function Badge({ children, tone, size, className }) {
    return _jsx("span", { className: cn(badgeVariants({ tone, size }), className), children: children });
}
