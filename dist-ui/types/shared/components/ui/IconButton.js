...
const iconButtonClass = cva('focus-ring inline-flex items-center justify-center transition', {
    variants: {
        ...
        variant: {
            default: 'border border-[var(--color-line)] bg-[var(--color-surface)] text-[var(--color-text)] hover:bg-[var(--color-surface-elevated)]',
            ...
            overlay: 'rounded-full border border-[var(--color-line)] bg-[var(--color-overlay)] text-[var(--color-text)] backdrop-blur-sm transition-colors hover:border-[var(--color-accent)] hover:text-[var(--color-accent)] disabled:opacity-40',
        },
    },
    defaultVariants: { size: 'md', variant: 'default' },
});
export function IconButton({ className, type, size, variant, ...props }) {
    return (_jsx("button", { type: type ?? 'button', className: cn(iconButtonClass({ size, variant }), className), ...props }));
}
