 ...
 */
export function sanitizeHref(raw, options) {
 ...
 */
export function upgradeHttpToHttps(href) {
 ...
 */
export function withShopifyImageWidth(src, width) {
    let url;
    try {
        url = new URL(src);
    }
    ...
    if (!SHOPIFY_CDN_HOST_RE.test(url.hostname))
        return src;
    url.searchParams.set('width', String(Math.round(width)));
    return url.toString();
}
export function isLikelySafeMediaSrc(raw) {
    if (typeof raw !== 'string')
        return false;
    const t = raw.trim();
    if (!t)
    ...
    if (/^data:(?:image|video)\/[\w.+-]+[;,]/i.test(t))
        return true;
