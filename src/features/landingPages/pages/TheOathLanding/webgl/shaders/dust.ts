 ...
 * Motes shy away from the pointer (`uPointer` world-space + force), drift slows
 * as scenes dim (`uLift`), and the field breathes faintly brighter on hover /
 * the finale (`uGlint`) — always subtle, never a glow layer.
 */
...
void main() {
  ...
  pos.y += sin(t * 0.7 + aPhase * 17.0) * 0.35 + fract(aPhase + uTime * 0.004 * aSpeed) * 0.0;
