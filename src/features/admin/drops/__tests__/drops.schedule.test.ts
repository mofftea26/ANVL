...
describe('drop schedule persistence', () => {
  ...
  it('throws when scheduling a drop missing from local storage', () => {
    expect(() =>
      scheduleDropActivation('missing-drop', new Date().toISOString()),
    ).toThrow(/not found in local storage/i)
  })
})
