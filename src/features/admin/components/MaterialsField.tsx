...
/**
 * Structured fabric-composition editor, shared by the products PDP editor and
 * the passport content wizard: each row carries a material name, a composition
 * percentage (0–100), a GSM weight (positive), and an optional card image (via
 * the media library). Rows add / delete / drag-reorder (with the keyboard
 * up/down fallback, same contract as {@link CareSelector}). A non-blocking hint
 * appears when the authored percentages don't sum to 100.
 */
export function MaterialsField({ materials, onChange, assets }: MaterialsFieldProps) {
