...
export function ChapterShelfCard({ chapter, onOpen }: ChapterShelfCardProps) {
  ...
  return (
    <button
      ...
      className="focus-ring group relative flex w-full flex-col overflow-hidden rounded-sm border border-[var(--color-line)] bg-[var(--color-surface)] text-left transition-colors duration-300 hover:border-[color-mix(in_oklab,var(--color-ember)_55%,var(--color-line))]"
    ...
    >
      <span className="relative block aspect-[3/4] w-full overflow-hidden bg-[radial-gradient(120%_90%_at_50%_15%,#15161a_0%,var(--color-bg)_70%)]">
        ...
        <span className="anvl-display pointer-events-none absolute left-4 top-3 text-[clamp(2rem,5vw,3.5rem)] leading-none text-[var(--color-ember)] opacity-90 mix-blend-screen">
          {number}
        </span>
      </span>

      <span className="relative flex flex-1 flex-col p-5">
        <span className="anvl-display text-[11px] tracking-[0.28em] text-[var(--color-ember-bright)]">
        ...
        <span className="mt-4 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-[var(--color-text)] transition-colors group-hover:text-[var(--color-ember-bright)]">
