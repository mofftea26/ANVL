-- Product `body.dropIds` stores app Drop.id (e.g. drop_the-oath), not anvl_drops UUID PK.
-- Casting those strings to uuid broke cms_publish_drop on save/activate.
...
COMMENT ON FUNCTION public.cms_publish_drop IS
  'Atomically publish a drop, demote prior actives, bump storefront revision. Resolves product dropIds via client_drop_id.';
