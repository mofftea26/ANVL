-- Remove draft/archived drop statuses; rename draft_body → body (editor source of truth).

-- Migrate existing rows before tightening the CHECK constraint.
UPDATE public.anvl_drops
SET status = 'inactive'
WHERE status IN ('draft', 'archived');

ALTER TABLE public.anvl_drops
  DROP CONSTRAINT IF EXISTS anvl_drops_status_check;

ALTER TABLE public.anvl_drops
  ADD CONSTRAINT anvl_drops_status_check
  CHECK (status IN ('active', 'inactive', 'scheduled'));

ALTER TABLE public.anvl_drops
  RENAME COLUMN draft_body TO body;

COMMENT ON COLUMN public.anvl_drops.body IS 'Full drop JSON from the CMS editor (source of truth until publish).';

-- Refresh client_drop_id from renamed column (triggered by prior migration logic).
UPDATE public.anvl_drops AS d
SET client_drop_id = NULLIF(trim(d.body ->> 'id'), '')
WHERE client_drop_id IS NULL OR client_drop_id = '';
